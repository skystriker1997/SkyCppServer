...
void Poller::UpdateChannel(Channel * channel) {
    	...
    	if(!channel->CheckInEpoll()) {
        		if(epoll_ctl(epfd_, EPOLL_CTL_ADD, fd, &ev) == -1) {
            			...
            			fd_count_ += 1;
                		message = message + std::to_string(epfd_) + " picked up sockfd." + std::to_string(fd) + ", which listens " + std::to_string(channel->GetListenEvents());
						logger_.DEBUG(message.c_str());
       			}
    	} else {
        		if(epoll_ctl(epfd_, EPOLL_CTL_MOD, fd, &ev) == -1) {
                		message = message + std::to_string(epfd_) + " failed to modify sockfd." + std::to_string(fd) + ", error info: " + strerror(errno);
            			logger_.ERROR(message.c_str());
        		} else {
						message = message + std::to_string(epfd_) + " modified sockfd." + std::to_string(fd) + ", which listens " + std::to_string(channel->GetListenEvents());
				}
    	}
}
